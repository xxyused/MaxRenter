...
/**
 * 描    述：身份证号帮助类，提供身份证号的相关操作（格式验证、性别提取和生日提取和格式化身份证号）。
 * 作    者：向晓阳
 * 时    间：2016/2/25
 * 版    权：迈斯云门禁网络科技有限公司
 */
public class IdCardNoHelper {
     ...
     * 获取占位符个数。
     *
     * @param idCardNo 要查找的字符串。
     * @return
     */
    public static int getPlaceholderCount(CharSequence idCardNo) {
        int placeholderCount = 0;
        for (int i = 0; i < idCardNo.length(); i++) {
            if (isPlaceholder(idCardNo.charAt(i))) {
                placeholderCount++;
            }
        }
    ...
    /**
     * 格式化身份证号码，返回格式化后的身份证号码。
     *
     * @param idCardNo 要格式化的身份证号码。
     * @return 格式化后的身份证号码。
     */
    public static String formatIdCardNo(CharSequence idCardNo) {
     ...
     * 格式化身份证号码，返回新的光标位置。
     *
     * @param editable 要格式化的身份证号码。
     * @return 新的光标位置。
     */
    public static int formatIdCardNo(Editable editable, int selectionEnd) {
        int index = 0;
     ...
     * 解析身份证号码，去除占位符。
     *
     * @param fmtText 要解析的身份证号码。
     * @return 去除占位符后的身份证号码。
     */
    public static String parseIdCardNo(CharSequence fmtText) {
        if (null == fmtText) {
            return "";
        }
    ...
    private static boolean isPlaceholderIndex(int index) {
        return (index == 6 || index == 11 || index == 16);
    }
}
